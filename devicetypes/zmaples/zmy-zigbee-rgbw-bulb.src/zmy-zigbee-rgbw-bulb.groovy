...
metadata {
    definition (name: "ZMy ZigBee RGBW Bulb", namespace: "zmaples", author: "zmaples") {
    ...
    if (event) {
        ...
        if (zigbeeMap?.clusterInt == COLOR_CONTROL_CLUSTER) {
            if(zigbeeMap.attrInt == ATTRIBUTE_HUE){  //Hue Attribute
                def hueValue = Math.round(zigbee.convertHexToInt(zigbeeMap.value) / 255 * 360)     //ZM-PUSHED BACK TO 0-360 to more accurately convert
...
def configure() {
    ...
    // Device-Watch allows 3 check-in misses from device + ping (plus 1 min lag time)
...
def setHue(value) {
    def scaledHueValue = zigbee.convertToHexString(Math.round(value * 0xfe / 360.0), 2)    //ZM-PUSHED BACK TO 0-360 to more accurately convert
